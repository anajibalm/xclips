...
function StudioContent() {
  ...
  // Subtitle timing offset calibration in milliseconds (unlimited positive/negative range)
  ...
  // Google Font Loader (20 curated fonts)
  useEffect(() => {
  ...
  // Synchronize time when selected clip changes (keyed on clip ID and startSec to prevent render cycles)
  ...
  }, [selectedClip?.id, selectedClip?.startSec]);
  ...
  const selectedClipRef = useRef<XclipsClip | null>(selectedClip);
  selectedClipRef.current = selectedClip;
  const projectDurationRef = useRef<number>(project?.durationSec || 0);
  ...
  }, [activePhraseIndex, autoScrollToPlayhead]);
  ...
  // Effective playback time for subtitles considering user-calibrated timing offset (ms converted to seconds)
  const effectiveSubtitleTime = currentTime - subtitleOffsetMs / 1000;

  // Active word in current playback time with calibrated offset
  ...
  // Quick shift handler for subtitle timing calibration in milliseconds
  const handleShiftSubtitleOffsetMs = (deltaMs: number) => {
    setSubtitleOffsetMs((prev) => Math.round(prev + deltaMs));
  };

  // Apply offset permanently to all words and save to database
  const handleApplyOffsetPermanently = async () => {
    if (subtitleOffsetMs === 0 || editableWords.length === 0) return;
    setIsSavingTranscript(true);
    setActionError(null);
    setActionSuccess(null);
    ...
    if (res.ok && res.data?.transcript) {
      ...
      setActionSuccess(
        `Kalibrasi offset ${appliedMs > 0 ? `+${appliedMs}` : appliedMs} ms (${shiftAmountSec > 0 ? `+${shiftAmountSec}` : shiftAmountSec}s) berhasil diterapkan permanen ke seluruh transkrip dan database!`
      );
      setTimeout(() => setActionSuccess(null), 4000);
      fetchAssets();
      fetchLogs();
    } else {
      setActionError(res.data?.message || "Gagal menyimpan kalibrasi offset transkrip");
    }
  };
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* TAB 1: SUBTITLES & SYNC STUDIO (2-COLUMN DUAL-PANE LAYOUT) */}
              {activeTab === 1 && (
                          ...
                          <Box sx={{ height: itemPositions.totalHeight, width: "100%", position: "relative" }}>
                            {filteredPhrases
                              ...
                              .map((seg, sliceIdx) => {
                                ...
                                // Strict single active phrase matching effectiveSubtitleTime
                  ...
                  {!selectedClip ? (
                    ...
                    <Box>
                      ...
                      {currentStyle.enabled === false ? (
                        ...
                        <Box>
                          {/* SYNC & TIMING CALIBRATION SECTION IN STYLE TAB (MILLISECONDS INPUT) */}
                          <Box
                            sx={{
                              p: 1.8,
                              bgcolor: subtitleOffsetMs !== 0 ? "rgba(245, 158, 11, 0.08)" : "#141418",
                              borderRadius: 1,
                              border: subtitleOffsetMs !== 0 ? "1px solid rgba(245, 158, 11, 0.35)" : "1px solid #27272a",
                              mb: 3,
                            }}
                          >
                            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1.2, flexWrap: "wrap", gap: 1 }}>
                              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                ...
                                <Typography variant="subtitle2" sx={{ fontWeight: 800, color: "#fafafa", fontSize: "0.85rem" }}>
                                  1. Kalibrasi Waktu Subtitle (Milidetik)
                                </Typography>
                              </Box>
                              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                <Chip
                                  label={
                                    subtitleOffsetMs === 0
                                      ? "Tepat (0 ms)"
                                      : subtitleOffsetMs > 0
                                      ? `Tunda ${subtitleOffsetMs} ms (+${(subtitleOffsetMs / 1000).toFixed(2)}s)`
                                      : `Maju ${Math.abs(subtitleOffsetMs)} ms (${(subtitleOffsetMs / 1000).toFixed(2)}s)`
                                  }
                                  size="small"
                                  sx={{
                                    bgcolor: subtitleOffsetMs !== 0 ? "#f59e0b" : "#27272a",
                            ...
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1.2, flexWrap: "wrap", mb: 1 }}>
                              <TextField
                                ...
                                slotProps={{
                                  input: {
                                    endAdornment: <InputAdornment position="end"><Typography variant="caption" sx={{ color: "#71717a", fontWeight: 700 }}>ms</Typography></InputAdornment>,
                                  },
                                }}
                                sx={{
                                  width: { xs: "100%", sm: 170 },
                              ...
                              {/* Quick Shift Nudge Buttons in Milliseconds */}
                              <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, flexWrap: "wrap", flex: 1 }}>
                                <Button
                                  size="small"
                                  variant="outlined"
                                  onClick={() => handleShiftSubtitleOffsetMs(-1000)}
                                  sx={{ py: 0.3, px: 0.8, fontSize: "0.7rem", borderColor: "#3f3f46", color: "#e4e4e7", textTransform: "none", borderRadius: 0.8 }}
                                >
                                  -1000 ms
                                </Button>
                                <Button
                                ...
                                <Button
                                  ...
                                  onClick={() => setSubtitleOffsetMs(0)}
                                  disabled={subtitleOffsetMs === 0}
                                  sx={{ py: 0.3, px: 0.8, fontSize: "0.72rem", color: "#a1a1aa", textTransform: "none", fontWeight: 700 }}
                                >
                                  Reset (0)
                                </Button>
                            ...
                            <Typography variant="caption" sx={{ color: "#71717a", fontSize: "0.7rem", display: "block" }}>
                              Sesuaikan sinkronisasi subtitle jika video mendahului audio atau subtitle terlambat muncul.
                            </Typography>
                          </Box>
